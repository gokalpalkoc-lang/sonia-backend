  ...
  return {
    ...
    configureServer(server: ViteDevServer) {
      server.middlewares.use("/api/commands", async (req, res) => {
            ...
            try {
              ...
              try {
                ...
                if (createAssistantResponse.ok) {
                  const assistantData =
                    (await createAssistantResponse.json()) as { id: string };
                  assistantId = assistantData.id;
                  console.log("✅ Created new assistant:", assistantId);
                } else {
                  console.error(
                    "Failed to create assistant:",
                    createAssistantResponse.status,
                  );
                }
              } catch (error) {
        ...
        if (
          req.method === "PUT" &&
          (req.url === "/called" || req.url === "/api/commands/called")
        ) {
          let body = "";
          req.on("data", (chunk) => {
            body += chunk.toString();
          ...
          req.on("end", () => {
            try {
              ...
              if (assistantId && date) {
                ...
                console.log(
                  `📅 Updated last called date for ${assistantId}: ${date}`,
                );
                res.writeHead(200, { "Content-Type": "application/json" });
                ...
                res.end(
                  JSON.stringify({
                    success: false,
                    error: "Missing assistantId or date",
                  }),
                );
              }
            } catch (error) {
        ...
        if (
          req.method === "GET" &&
          (req.url?.startsWith("/last-called/") ||
            req.url?.startsWith("/api/commands/last-called/"))
        ) {
